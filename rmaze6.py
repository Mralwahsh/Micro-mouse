...
def main():
    ...
    # Updated Mouse Colors
    COLOR_PHYSICAL_MOUSE = (0, 150, 255) # Blue square
    COLOR_MEMORY_MOUSE = (0, 255, 0)     # Bright Green circle (from image)
    COLOR_VISITED_TRAIL = (70, 0, 130)   # Dark purple heat map (from image)
    COLOR_SOLUTION = (255, 0, 0)         # Bright red path (from image)
    ...
    def draw_maze(offset_x, offset_y, title_text, maze_data, is_memory=False):
        ...
        if not is_memory:
            ...
            # --- MEMORY MAP DRAWING (Matches your image) ---
            ...
            for x in range(COLS):
                for y in range(ROWS):
                    ...
                    if cell.discovered:
                        # Draw purple trail for visited cells
                        if (x, y) in visited:
    ...
    while running:
        ...
        if explore_state == "EXPLORING":
            if (mouse_x, mouse_y) in target_cells:
                ...
                # 1. Update Sensors (Fixed: Now updates BOTH sides of the wall in memory)
                ...
                for d, (dx, dy) in dirs.items():
                    ...
                    # Update the neighbor's shared wall
                ...
                if unvisited_neighbors:
                    # Priority Override: If the goal is right next to us, jump in immediately!
        ...
        if explore_state in ["EXPLORING", "PAUSED"]:
            mouse_px = 40 + WALL_THICKNESS + (mouse_x * (CELL_SIZE + WALL_THICKNESS)) + 4
            mouse_py = 50 + WALL_THICKNESS + (mouse_y * (CELL_SIZE + WALL_THICKNESS)) + 4
            pygame.draw.rect(screen, COLOR_PHYSICAL_MOUSE, (mouse_px, mouse_py, CELL_SIZE - 8, CELL_SIZE - 8), border_radius=4)

        # Draw the Memory Mouse token (Green Circle on Right)
        if explore_state in ["EXPLORING", "PAUSED"]:
            center_x = right_offset + WALL_THICKNESS + (mouse_x * (CELL_SIZE + WALL_THICKNESS)) + CELL_SIZE // 2
        ...
        pygame.display.flip()
        clock.tick(20) # Slightly slower FPS so you can watch the DFS logic work
