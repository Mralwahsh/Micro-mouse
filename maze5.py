...
# this find the shortest path using optimized BFS to find the shortest path without trying every single one
...
# ==========================================
def solve_maze_fast(maze, start_x, start_y, end_x, end_y):
    # deque is much faster than a standard list for queue operations
    # We store: (current_x, current_y, path_taken_to_get_here)
    queue = deque([(start_x, start_y, [(start_x, start_y)])])
    
    # Track visited cells so we never look at dead ends twice
    visited = set([(start_x, start_y)])
    
    while queue:
        x, y, current_path = queue.popleft()
        
        # The first time we hit the end, it is guaranteed to be the shortest path
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if regen_btn.collidepoint(event.pos):
                    ...
                    # Boom. Instant shortest path.
                    solution_path = solve_maze_fast(current_maze, 0, 1, GRID_W - 1, GRID_H - 2)
                    
                    if solution_path:
                        print(f"Shortest path found in {len(solution_path)} steps!")
