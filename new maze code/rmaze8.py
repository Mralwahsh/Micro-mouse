...
def main():
    ...
    # Calculate the absolute shortest path on the physical maze instantly
    absolute_shortest_path = get_shortest_known_path(current_maze, 0, 9, target_cells)
    
    mouse_x, mouse_y = 0, 9 
    path_stack = [] 
    
    # Dual tracking for the 3-round system
    round_visited = set()   # Resets every round to allow fresh paths
    global_visited = set()  # Persists across rounds to force exploration of new branches
    ...
    def draw_maze(offset_x, offset_y, title_text, maze_data, is_memory=False, show_trail=False, path_to_draw=None):
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if regen_btn.collidepoint(event.pos):
                    ...
                    if explore_state == "IDLE":
                        ...
                        round_visited = set([(mouse_x, mouse_y)])
                        global_visited.add((mouse_x, mouse_y))
                        # Note: mouse_memory and global_visited are INTENTIONALLY not cleared
            ...
            if (mouse_x, mouse_y) in target_cells:
                ...
                if valid_neighbors:
                    ...
                    if jump_to_goal:
                        ...
                        # Sort neighbors: those NOT in global_visited come first (0), else (1)
                    ...
                    # Dead end, backtrack physically
                    if len(path_stack) > 1:
                        path_stack.pop()
                        mouse_x, mouse_y = path_stack[-1]
                    else:
                        # Reached start with nowhere else to go
                        explore_state = "ROUND_PAUSED" if current_round < max_rounds else "DONE"

        # Calculate BFS shortest path on memory map dynamically
        ...
        # 4. Absolute Shortest Path (Far Right)
        offset_4 = offset_3 + MAZE_PIXEL_WIDTH + PADDING
        draw_maze(offset_4, 50, "Absolute Shortest Path", current_maze, is_memory=False, show_trail=False, path_to_draw=absolute_shortest_path)
        ...
        pygame.display.flip()
        clock.tick(30) # Fast enough to not be boring, slow enough to watch the branching logic
