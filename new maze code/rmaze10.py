...
def main():
    ...
    WINDOW_HEIGHT = MAZE_PIXEL_HEIGHT + 160 # Increased height to fit telemetry text
    ...
    COLOR_TEXT_METRIC = (180, 220, 255) # Light blue for telemetry text
    ...
    # Track exactly how many cells the mouse physically drove into per round
    round_steps = [0, 0, 0] 
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if regen_btn.collidepoint(event.pos):
                    ...
                    round_steps = [0, 0, 0] # Reset steps tracking
        ...
        if explore_state == "EXPLORING":
            if (mouse_x, mouse_y) in target_cells:
                ...
                if valid_neighbors:
                    ...
                    # Track that the mouse drove into a new cell
                    round_steps[current_round - 1] += 1
                else:
                    if len(path_stack) > 1:
                        path_stack.pop()
                        mouse_x, mouse_y = path_stack[-1]
                        
                        # Track that the mouse had to drive backward to backtrack
                        round_steps[current_round - 1] += 1
                    else:
        ...
        # Calculate exact center points under each maze for text rendering
        ...
        draw_centered_text(f"Live Drive: {round_steps[current_round-1]} cells", center_1, text_y_pos)
        ...
        draw_centered_text(f"Search Steps: R1: {round_steps[0]} | R2: {round_steps[1]} | R3: {round_steps[2]}", center_2, text_y_pos)

        # 3. Memory Fast Run Phase
        ...
        current_best_len = len(solution_path) if solution_path else 0
        draw_centered_text(f"Found Route Length: {current_best_len} cells", center_3, text_y_pos)

        # 4. Absolute Shortest Path
        ...
        draw_centered_text(f"Perfect Route Length: {perfect_len} cells", center_4, text_y_pos)
